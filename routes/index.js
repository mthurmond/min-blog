...
app.myApp.use(async (req, res, next) => {
    if (res.locals.userId) {
        ...
        if (user.photo) {
            res.locals.photo = `https://dbn4sin1xpjx7.cloudfront.net/profile-pictures/${user.photo}`
        }
    }
    next();
});
...
router.get('/:username/:slug', async (req, res, next) => {
    try {
        ...
        const userPhoto = author.photo ? `https://dbn4sin1xpjx7.cloudfront.net/profile-pictures/${author.photo}` : defaultAvatar
...
router.get('/:username', async (req, res, next) => {
    try {
        ...
        const authorPhoto = author.photo ? `https://dbn4sin1xpjx7.cloudfront.net/profile-pictures/${author.photo}` : defaultAvatar
